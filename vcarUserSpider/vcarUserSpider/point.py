...
class Point(object):
    ...
    def cutIntoLastElement(cls):
        ...
        if hasOverFile:
            ...
            if len(lines) - 1 == lines.rfind(","):
                lines = lines[0:lines.rfind(",")]
            # 提取已爬取的车型id，封装成set集合
            idList = list(lines.split(","))
            lastElement=idList[len(idList)-1]
    ...
    # 删除断点文件中原有的数据，写入新的数据
    @classmethod
    def clearAndSavePoint(cls,point):
        pointFile=open(cls.pointFilePath,"w",encoding="utf-8")
